...
test.describe("Test suite for Input tests",
    { tag: "@input-all" }, () => {
        test("Input", { tag: ["@input-fillout-fields"] }, async ({ testPage, inputPage }) => {
            ...
            await test.step("Fill out input fields", async () => {
                await inputPage.fillOutFullName("William Ribeiro Moreno");
                await inputPage.appendTextNPressTab("Text to append");
                await inputPage.getTextFromWhatIsInsideTheTextBox();
                await inputPage.clearText();
                await inputPage.expectDisabled();
                await inputPage.expectReadOnly();
            });
        })
    });
